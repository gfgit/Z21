...
void z21Class::receive(uint8_t client, uint8_t *packet)
{
  ...
  case (LAN_X_Header):
  {
    ...
    case LAN_X_CV_POM:
    {
      ...
      if (packet[5] == 0x30)
      {
        ...
        if ((packet[8] & 0xFC) == LAN_X_CV_POM_WRITE_BYTE)
        {
          // DB3 Option 0xEC
#if defined(SERIALDEBUG)
        ...
        else if ((packet[8] & 0xFC) == LAN_X_CV_POM_WRITE_BIT)
        {
          // DB3 Option 0xE8
#if defined(SERIALDEBUG)
        ...
        else if ((packet[8] & 0xFC) == LAN_X_CV_POM_READ_BYTE)
        {
          // DB3 Option 0xE4
#if defined(SERIALDEBUG)
      ...
      {
        // DB0 = LAN_X_CV_POM_ACCESSORY
        uint16_t Adr = ((packet[6] & 0x1F) << 8) + packet[7];
        if ((packet[8] & 0xFC) == LAN_X_CV_POM_ACCESSORY_WRITE_BYTE)
        {
          // DB3 Option 0xEC
#if defined(SERIALDEBUG)
        ...
        else if ((packet[8] & 0xFC) == LAN_X_CV_POM_ACCESSORY_WRITE_BIT)
        {
          // DB3 Option 0xE8
#if define
          d(SERIALDEBUG) ZDebug.println("LAN_X_CV_POM_ACCESSORY_WRITE_BIT");
#endif
          if (notifyz21CVPOMACCWRITEBIT)
            notifyz21CVPOMACCWRITEBIT(Adr, CVAdr, value); // set Bit
        }
        else if ((packet[8] & 0xFC) == LAN_X_CV_POM_ACCESSORY_READ_BYTE)
        {
          // DB3 Option 0xE4
#if defined(SERIALDEBUG)
    ...
    {
      // and notify other Clients with LAN_X_GET_TURNOUT_INFO!
#if defined(SERIALDEBUG)
    ...
    case LAN_X_GET_EXT_ACCESSORY_INFO:
    {
      ...
      ZDebug.print((packet[5] << 8) + packet[6]);
      ZDebug.print(":0x");
      ZDebug.println(packet[7], HEX); // DB2 Reserved for future extensions
#endif
      if (LAST_EXTACC_received == true)
      {
        // 0x00 ... Data Valid;
        setExtACCInfo((packet[5] << 8) + packet[6], LAST_EXTACC_msg);
      }
      else
      {
        // 0xFF ... Data Unknown
        setExtACCInfo((packet[5] << 8) + packet[6], LAST_EXTACC_msg, 0xFF);
      }
      break;
    }
    case LAN_X_SET_STOP:
    ...
    case LAN_X_SET_LOCO:
    {
      ...
      else if (packet[5] == LAN_X_SET_LOCO_FUNCTION)
      {
        // DB0 = 0xF8
        // LAN_X_SET_LOCO_FUNCTION  Adr_MSB        Adr_LSB            Type (00=AUS/01=EIN/10=UM)
        // Funktion
        if (notifyz21LocoFkt)
          notifyz21LocoFkt(word(packet[6] & 0x3F, packet[7]), packet[8] >> 6,
                           packet[8] & 0b00111111);
        // uint16_t Adr, uint8_t type, uint8_t fkt
      }
      // LAN_X_SET_LOCO_FUNCTION_GROUP:
      else if ((packet[5] == 0x20) && notifyz21LocoFkt0to4)
        notifyz21LocoFkt0to4(word(packet[6] & 0x3F, packet[7]),
                             packet[8] & 0x1F); // 0 0 0 F0 F4 F3 F2 F1
      else if ((packet[5] == 0x21) && notifyz21LocoFkt5to8)
        notifyz21LocoFkt5to8(word(packet[6] & 0x3F, packet[7]),
                             packet[8] & 0x0F); // 0 0 0 0 F8 F7 F6 F5
      else if ((packet[5] == 0x22) && notifyz21LocoFkt9to12)
        notifyz21LocoFkt9to12(word(packet[6] & 0x3F, packet[7]),
                              packet[8] & 0x1F); // 0 0 0 0 F12 F11 F10 F9
      else if ((packet[5] == 0x23) && notifyz21LocoFkt13to20)
        notifyz21LocoFkt13to20(word(packet[6] & 0x3F, packet[7]),
                               packet[8]); // F20 F19 F18 F17 F16 F15 F14 F13
      else if ((packet[5] == 0x28) && notifyz21LocoFkt21to28)
        notifyz21LocoFkt21to28(word(packet[6] & 0x3F, packet[7]),
                               packet[8]); // F28 F27 F26 F25 F24 F23 F22 F21
      else if ((packet[5] == 0x29) && notifyz21LocoFkt29to36)
        notifyz21LocoFkt29to36(word(packet[6] & 0x3F, packet[7]),
                               packet[8]); // F36 F35 F34 F33 F32 F31 F30 F29
      else if (packet[5] == 0x2A)
      {
        // F44 - F37
  ...
  case (LAN_LOCONET_DETECTOR):
  {
    if (notifyz21LNdetector)
    {
      ...
      // Request Type & Report Address
      notifyz21LNdetector(client, packet[4], word(packet[6], packet[5]));
    }
    break;
  }
  case (LAN_CAN_DETECTOR):
  {
    if (notifyz21CANdetector)
    {
      ...
      // Request Type & CAN-ID
      notifyz21CANdetector(client, packet[4], word(packet[6], packet[5]));
    }
    break;
  }
  case (0x12):
...
//--------------------------------------------------------------------------------------------
// Status feedback to Z21 device - mark locomotive as Busy!
...
void z21Class::returnLocoStateFull(byte client, uint16_t Adr, bool bc)
{
  ...
  uint8_t ldata[6];
  if (notifyz21LocoState)
    notifyz21LocoState(Adr, ldata); // uint8_t Steps[0], uint8_t Speed[1], uint8_t F0[2], uint8_t
                                    // F1[3], uint8_t F2[4], uint8_t F3[5]
...
{
  if (client > 0)
    EthSend(client, 0x04 + DataLen, LAN_LOCONET_DETECTOR, data, false,
            Z21bcNone); // LAN_LOCONET_DETECTOR
  else
    EthSend(0, 0x04 + DataLen, LAN_LOCONET_DETECTOR, data, false,
            Z21bcLocoNet_s); // LAN_LOCONET_DETECTOR
}
...
bool z21Class::setLNMessage(byte *data, byte DataLen, byte bcType, bool TX)
{
  ...
  {
    // LAN_LOCONET_Z21_TX
    EthSend(0, 0x04 + DataLen, LAN_LOCONET_Z21_TX, data, false, bcType);
  }
  else
  {
    // LAN_LOCONET_Z21_RX
    EthSend(0, 0x04 + DataLen, LAN_LOCONET_Z21_RX, data, false, bcType);
  }
  return true;
}
...
void z21Class::setCANDetector(uint16_t NID, uint16_t Adr, uint8_t port, uint8_t typ, uint16_t v1,
                              uint16_t v2)
{
  byte data[10];
  data[0] = NID & 0xFF;
...
void z21Class::sendSystemInfo(byte client, uint16_t maincurrent, uint16_t mainvoltage,
                              uint16_t temp)
{
  byte data[16];
  ...
  */
  ...
  // only to the request client if specified or
  // if client = 0 to all that select this message (subscribed)!
  if (client > 0)
    EthSend(client, 0x14, LAN_SYSTEMSTATE_DATACHANGED, data, false, Z21bcNone);
  else
    EthSend(0, 0x14, LAN_SYSTEMSTATE_DATACHANGED, data, false, Z21bcSystemInfo_s);
}
